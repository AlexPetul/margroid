    ...
    $.ajax({
        ...
        success: function(data){
            ...
            $.each(clear_data, function(index, color_item){
                ...
                '<div class="col-xl-6">\
                    <label product-size-id="' + color_item[5] + '" class="image-radio" prev-price="' + color_item[4] + '" color-price="' + color_item[3] + '">\
  ...
  $(".image-radio").each(function(){
    ...
    $('.item-add-to-card').on('click', function(){
        ...
        if (if_sizes_exists && !size_chosen) {
            ...
            var color_id = $('.image-radio-checked').attr('product-size-id');
            data = {
                product_id: product_id,
                color_id: color_id,
                csrfmiddlewaretoken: csrftoken
            ...
            $.ajax({
                ...
                success: function(data){
                  ...
                  $('#cart-total-mobile').html(data.total);
    ...
    $('#bank-payment-left').on('change', function(){
        $('#bank-payment-form').fadeIn(400);
    });

    $('.not-bank-payment').on('change', function(){
        $('#bank-payment-form').css('display', 'none');
    });

    $('#after-mkad-left').on('change', function(){
        $('#transport-company-form').css('display', 'none');
        $('#before-mkad-form').css('display', 'none');
        $('#take-away-form').css('display', 'none');
        $('#after-mkad-form').fadeIn(500);
    });

    $('#before-mkad-left').on('change', function(){
        $('#transport-company-form').css('display', 'none');
        $('#after-mkad-form').css('display', 'none');
        $('#take-away-form').css('display', 'none');
        $('#before-mkad-form').fadeIn(500);
    });

    $('#take-away-left').on('change', function(){
        $('#transport-company-form').css('display', 'none');
        $('#before-mkad-form').css('display', 'none');
        $('#after-mkad-form').css('display', 'none');
        $('#take-away-form').fadeIn(500);       
    });

    $('#transport-company-left').on('change', function(){
        $('#before-mkad-form').css('display', 'none');
        $('#after-mkad-form').css('display', 'none');
        $('#take-away-form').css('display', 'none');
        $('#transport-company-form').fadeIn(500);
    });
    ...
    $('.view-order').on('click', function(){
        ...
        $('#modal-order-' + order_id).modal('toggle');
    });
