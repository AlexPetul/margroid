...
class ContactForm(forms.Form):
	...
	message = forms.CharField(label='Сообщение', widget=forms.Textarea(attrs={'placeholder': 'Напишите свое сообщение и мы обязательно Вам поможем!'}))
...
class ComplectForProductsForm(forms.ModelForm):
	complect = forms.ModelMultipleChoiceField(queryset=Product.objects.filter(category__name='Комплектующие'), label='Комплектующие', required=False)