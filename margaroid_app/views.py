...
def product_detail_view(request, product_id):
	current_product = Product.objects.get(id=product_id)
	current_product_sizes = Sizes.objects.filter(product=current_product)
	...
	if request.GET:
		if request.GET.get('from') and request.GET.get('to'):
			...
			products = Product.objects.filter(category=current_category, \
				sub_category=current_subcategory, price__lte=to_price, price__gte=from_price)
		else:
...
def search_view(request):
	...
	if 'category' in request.GET:
		...
		if searching_category == 'Все категории':
			...
			founded_products = Product.objects.filter(name__icontains=searching_query, category=filtered_category, sub_category=filtered_subcategory)
	else:
...
def make_order_view(request):
	...
	additional_informations = [request.POST.get('additional-information-1'), request.POST.get('additional-information-2'),
	request.POST.get('additional-information-3'), request.POST.get('additional-information-4')]
	...
	new_order = Order.objects.create(user=user, name=first_name, full_name=full_name, phone=phone, email=email,
		...
		delivery_city=city, delivery_street=street, delivery_house=house_number, delivery_padik=padik,
		delivery_flat=flat_number, total_price=total_cart_price, delivery_deadline=deadline,
...
def send_feedback_view(request):
	...
	send_mail("Новое сообщение от margroid-msk.ru", "{0}.\nОт {1} ({2})".format(message, email_from, name), settings.EMAIL_HOST_USER, [settings.EMAIL_HOST_USER])	
	return JsonResponse({})